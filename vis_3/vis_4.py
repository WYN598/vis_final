...
fig = make_subplots(
    ...
    cols=4,
    shared_xaxes=True,
    row_heights=[0.8, 0.01, 0.2, 0.4, 0.5],  # 调整每行高度比例
    specs=[
        [{"rowspan": 2, "colspan": 4}, None, None, None],  # 蜡烛图行
        [None, None, None, None],  # 被蜡烛图占用的行
        [{"rowspan": 1, "colspan": 2}, None, {"colspan": 2}, None],  # RSI和成交量行
        [{"rowspan": 1, "colspan": 4, "type": "domain"}, None, None, None],  # 修改这行，让 Treemap 占据整行
        [{"rowspan": 1, "colspan": 1, "type": "domain"}, {"rowspan": 1, "colspan": 1, "type": "domain"}, {"rowspan": 1, "colspan": 2}, None],  # 饼图和热力图
    ]
)
date_buttons = [
